...
const SimpleDropdown = ({ label, options, value, onChange, displayKey = 'name', placeholder = 'Select...', className = '', multi = false }) => {
    ...
    return (
        <div className={`relative min-w-[180px] ${className}`} ref={ref}>
            ...
            <button
                type="button"
                className={`w-full px-3 py-2 bg-[#F3F4F9] rounded-lg text-left flex items-center justify-between text-sm font-medium text-[#94A3B8] hover:bg-gray-200 focus:outline-none ${open ? 'shadow-lg' : ''}`}
