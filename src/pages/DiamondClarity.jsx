...
import { fetchDiamondClarity, updateDiamondClarity, statusChange } from '../redux/services/diamondClarityservice';

const getColumns = (handleEdit, handleToggleStatus, statusLoadingId) => [
  { key: 'name', title: 'Diamond Clarity' },
  {
    key: 'status',
    title: 'Status',
    align: 'center',
...
const DiamondClarity = () => {
  ...
  const loadDiamondClarity = async (pageNum = 1) => {
    setLoading(true);
    try {
      const response = await fetchDiamondClarity({ page: pageNum, limit: 10, search: '' });
      const allDocs = (response?.Data?.docs || []).map(item => ({ ...item, name: item.grade }));
      if (pageNum === 1) {
        setData(allDocs);
      } else {
        setData(prev => [...prev, ...allDocs]);
      }
      setHasMore(allDocs.length > 0);
    } catch {
      setHasMore(false);
    } finally {
      setLoading(false);
  ...
  const handleToggleStatus = async (row) => {
    ...
    setStatusError('');
    try {
      await statusChange({ diamondclaritiesId: row._id });
      loadDiamondClarity(1);
    } catch (error) {
  ...
  return (
    <>
      ...
      />
      {showModal && <AddDiamondClarityModal
        onClose={() => { setShowModal(false); setSelectedRow(null); }}
        diamondClarityData={selectedRow}
        onSuccess={() => {
          setPage(1);
          loadDiamondClarity(1);
        }}
